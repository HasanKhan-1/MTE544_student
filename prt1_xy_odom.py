...
parser = argparse.ArgumentParser(description='Plot odometry vs particle filter positions')
...
fig.suptitle('Odometry vs Particle Filter Positions', fontsize=16, fontweight='bold')
...
for idx, (part_name, file_path) in enumerate(files.items()):
    ...
    try:
        ...
        for col in columns:
            ...
            if 'odom' in col_clean and 'x' in col_clean:
                col_map['odom_x'] = col
            elif 'odom' in col_clean and 'y' in col_clean:
                col_map['odom_y'] = col
            elif 'pf' in col_clean and 'x' in col_clean:
                col_map['pf_x'] = col
            elif 'pf' in col_clean and 'y' in col_clean:
                col_map['pf_y'] = col
        
        # Extract x and y positions
        odom_x = df[col_map['odom_x']]
        odom_y = df[col_map['odom_y']]
        pf_x = df[col_map['pf_x']]
        pf_y = df[col_map['pf_y']]
        
        # Plot on corresponding subplot
        ax.plot(odom_x, odom_y, 'b-', label='Odometry', linewidth=2, alpha=0.7)
        ax.plot(pf_x, pf_y, 'r-', label='Particle Filter', linewidth=2, alpha=0.7)
        
        # Mark start and end points
        ax.plot(odom_x.iloc[0], odom_y.iloc[0], 'go', markersize=10, label='Start')
        ax.plot(odom_x.iloc[-1], odom_y.iloc[-1], 'ks', markersize=10, label='End')
...
plt.tight_layout()
plt.savefig('odom_vs_pf_comparison.png', dpi=300, bbox_inches='tight')
print("\nPlot saved as 'odom_vs_pf_comparison.png'")
plt.show()